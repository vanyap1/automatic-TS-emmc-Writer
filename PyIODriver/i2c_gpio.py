...
class I2CGPIOController(Thread):
    ...
    def scanI2CBus(self):
        ...
        Returns:
        list: A list of I2C addresses of detected devices.
        """
        found_devices = []
        for address in range(0x03, 0x78):  # Стандартний діапазон I2C адрес
            try:
                self.bus.write_quick(address)
                found_devices.append(address)
            except OSError:
                pass  # Пристрій не відповідає на цю адресу
    ...
    def run(self):
        ...
        for hwExpander in self.expanders:
            if hwExpander.type == "PCA9535":
                print(f"i2c ADDR {hex(hwExpander.addr)}, {hwExpander.ioDir}")
